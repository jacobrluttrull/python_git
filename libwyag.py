...
def cmd_rev_parse(args):
    ...
    print(object_find(repo, args.name, fmt=fmt))
...
def tree_to_dict(repo, ref, prefix=""):
    ret = dict()
    tree_sha = object_read(repo, ref, fmt=b"tree")
...
#9 Staging area and index Part2 of staging and committing
...
argsp.add_argument("path", nargs="+", help="Files to remove from the index and the working tree.")
def cmd_rm(args):
    repo = repo_find()
    rm(repo, args)
...
argsp.add_argument("path", nargs="+", help="Files to add to the index.")
...
argsp = argsubparsers.add_parser("commit", help="Create a new commit object.")
argsp.add_argument("-m", dest="message", required=True, help="The commit message.")

def gitconfig_read():
    xdg_config_home = os.environ.get("XDG_CONFIG_HOME") if "XDG_CONFIG_HOME" in os.environ else "~/.config"
    config_files = [
